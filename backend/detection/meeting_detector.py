...
SILENCE_TIMEOUT = float(os.environ.get("AURELIUS_SILENCE_TIMEOUT", "120"))  # seconds of silence → auto-stop
...
class MeetingDetector:
    def __init__(self, app):
        ...
        self._streak = 0         # consecutive mic-in-use polls
    ...
    async def _tick(self):
        ...
        if recording:
            ...
            self._offered = False
            self._streak = 0
            if session.seconds_since_voice() >= SILENCE_TIMEOUT:
                logger.info(
                    f"Auto-stopping {session.meeting_id}: silent for "
                    f">{SILENCE_TIMEOUT:.0f}s"
                )
